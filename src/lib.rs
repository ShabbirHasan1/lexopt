...
impl Parser {
        ...
        match self.state {
            ...
            State::Shorts(ref arg, ref mut pos) => {
                ...
                match first_codepoint(&arg[*pos..]) {
                    ...
                    // Though if you have one you should maybe disable parse_short_equals.)
                    // clap always interprets it as a short flag in this case, but
                    // that feels sloppy.
                    Ok(Some('=')) if *pos > 1 && self.parse_short_equals => {
                        return Err(Error::UnexpectedValue {
    ...
    /// yield `"b"`, `"c"`.
    ///
    /// (If [`parse_short_equals`](Self::parse_short_equals) is disabled then `-a=b c` will yield
    /// `"=b"`, `"c"`.)
    ///
    /// # Errors
    ...
    fn raw_optional_value(&mut self) -> Option<(OsString, bool)> {
        match replace(&mut self.state, State::None) {
            ...
            State::Shorts(mut arg, mut pos) => {
                ...
                if arg[pos] == b'=' && self.parse_short_equals {
                    // -o=value.
            ...
            State::ShortsU16(arg, mut pos) => {
                ...
                let mut had_eq_sign = false;
                if arg[pos] == b'=' as u16 && self.parse_short_equals {
                    pos += 1;
                    had_eq_sign = true;
                }
    ...
    /// ```
    /// let mut parser = lexopt::Parser::from_env();
    /// parser.parse_short_equals(false);
    /// ```
    ///
    ...
    ///     parser.parse_short_equals(true);
    /// }
    /// # _ => (), }} Ok(()) }
    /// ```
    pub fn parse_short_equals(&mut self, on: bool) {
        self.parse_short_equals = on;
    }
}
